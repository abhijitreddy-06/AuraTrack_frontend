...
import { LendedEntry, createLendedEntry, deleteLendedEntry, getLendedEntries, updateLendedEntry } from '../services/lended';

export const LendMoneyScreen: React.FC = () => {
  const { colors, isDark } = useTheme();
  const router = useRouter();
  const [entries, setEntries] = useState<LendedEntry[]>([]);
  ...
  const loadEntries = async () => {
    try {
      const response = await getLendedEntries();
      setEntries(response.data);
    } catch (error) {
      Alert.alert('Could not load lended entries', error instanceof Error ? error.message : 'Please try again.');
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editingId) {
        const response = await updateLendedEntry(editingId, payload);
        setEntries((current) => current.map((entry) => entry.id === editingId ? response.data : entry));
      } else {
        const response = await createLendedEntry(payload);
  ...
  const handleEdit = (item: LendedEntry) => {
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          style: 'destructive',
          onPress: () => void (async () => {
            try { await deleteLendedEntry(id); setEntries((current) => current.filter((entry) => entry.id !== id)); }
  ...
  const renderItem = ({ item }: { item: LendedEntry }) => (
